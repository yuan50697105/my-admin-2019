...
public class AdminUserController extends BaseController {
    ...
    public Result<?> updateRole(@RequestBody @Validated(AdminUserFormVO.UpdateRole.class) AdminUserFormVO adminUserFormVO, BindingResult result) {
        validate(result);
        return adminUserService.updateUserRole(adminUserFormVO);
    }
    ...
    public Result<?> delete(@PathVariable("id") @NotNull(message = CommonsFormValidateConstants.ID_NOT_NULL_MSG) Long id) {
        return adminUserService.delete(Collections.singletonList(id));
    }

    @GetMapping("get")
    public Result<?> get(@RequestParam("id") @NotNull(message = CommonsFormValidateConstants.ID_NOT_NULL_MSG) Long id) {

...
public class SysUserServiceImpl implements SysUserService {
    ...
    public ResponseResult selectPage(SysUserCondition condition) {
        return data(sysUserDao.selectPage(condition));
    }

    @Override
    public ResponseResult selectList(SysUserCondition condition) {
        return data(sysUserDao.selectList(condition));
    }

    @Override
    public ResponseResult selectOne(SysUser condition) {
        return data(sysUserDao.selectOne(condition));
    }

    @Override
    public ResponseResult selectById(Long id) {
        return data(sysRoleDao.selectById(id));
    }
    ...
    public ResponseResult save(SysUserVo sysUserVo) {
        ...
        if (optional.isPresent()) {
            throw new ExistResultRuntimeException(ResponseResults.existError("username已存在"));
        }
    ...
    public ResponseResult updatePwd(SysUserVo sysUserVo) {
        ...
        if (!optional.isPresent()) {
            responseResult = ResponseResults.error(ResponseResults.ResultConstants.CHANGE_PASSWORD_USER_NOT_HAD, "用户不存在");
        } else {
            ...
            if (passwordEncoder.matches(oldPwd, password)) {
                ...
                responseResult = ResponseResults.error(ResponseResults.ResultConstants.CHANGE_PASSWORD_USER_OLD_PWD_ERROR, "密码不正确");
            }
        }
